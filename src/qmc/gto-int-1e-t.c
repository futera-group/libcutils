...
double gto_int_1e_t_core(double e1, unsigned *n1, double *pa,
  double e2, unsigned *n2, double *pb) {
  static unsigned i,j,m2[3];
  static double vm,vp,wm,wp,ss;
  vm = vp = wm = wp = 0.0;
  ss = gto_int_1e_s_core(e1,n1,pa,e2,n2,pb);
  for (i=0; i<3; i++) {
    for (j=0; j<3; j++)
      m2[j] = n2[j];
    m2[i] += 2;
    vp = gto_int_1e_s_core(e1,n1,pa,e2,m2,pb);
    wp += vp;
    if (n2[i]>1) {
      for (j=0; j<3; j++)
        m2[j] = n2[j];
      m2[i] -= 2;
      vm = gto_int_1e_s_core(e1,n1,pa,e2,m2,pb);
      wm += ((n2[i]*(n2[i]-1))*vm);
      }
    }
  return(e2*(2.0*(n2[0]+n2[1]+n2[2])+3.0)*ss-2.0*e2*e2*wp-0.5*wm);
  }
...
double gto_int_1e_t_bf(struct basis_shell *s1, double *c1, unsigned f1,
...
void gto_int_1e_t_mat(struct basis *b, double **m) {
        ...
        for (is2=0; is2<c2->n_shells; is2++) {
          ...
          for (if1=0; if1<s1->n_bfce; if1++) {
            ...
            for (if2=0; if2<s2->n_bfce; if2++) {
              ...
              if (id1<=id2) {
                m[id1][id2]= 
                 gto_int_1e_t_bf(s1,c1->coord,if1,s2,c2->coord,if2,rr,r2);
                if (id1!=id2)
                  m[id2][id1] = m[id1][id2];
                }
              }
