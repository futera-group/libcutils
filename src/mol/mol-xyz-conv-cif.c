...
/* convert xyz molecular data struct to cif struct
   ...
   box_a - box angles */
struct cif_mol *mol_xyz_cif(struct xyz_mol *x, short shift,
  ...
  if (shift || !box_s) {
    for (i=0; i<x->n_atoms; i++) {
      for (j=0; j<3; j++) {
        if (x->atom[i].coord[j]<min[j])
          min[j] = x->atom[i].coord[j];
        if (x->atom[i].coord[j]>max[j])
          max[j] = x->atom[i].coord[j];
        }
      }
    }
  /* unit cell */
  ...
  for (i=0; i<x->n_atoms; i++) {
    ...
    if (x->atom[i].name)
      ...
      c->atom[i].name = str_copy_new(atom_name(x->atom[i].num));
    for (j=0; j<3; j++) {
      crd[j] = x->atom[i].coord[j];
      if (shift)
        crd[j] -= min[j];
      crd[j] /= c->cell->side[j];
      }
